...
def _find_column_index(header_values: list, requested_name: str | None, candidates: list[str]) -> int | None:
    ...
    # Second pass: Partial matches for all candidates
    for c in candidates:
        n = _normalize_header(c)
        if not n: continue
        for i, v in indexed:
            vn = _normalize_header(v)
            if n in vn or vn in n:
                return i
...
def _clear_textarea(page, locator, attempts: int = 1):
    ...
    with sync_playwright() as p:
        ...
        for row_index in range(header_row + 1, int(ws.max_row or 0) + 1):
            ...
            try:
                ...
                # 1. Reset state
                if clear_locator and clear_locator.is_visible():
                    clear_locator.click()
                    page.wait_for_timeout(500)
                else:
                    _clear_textarea(page, input_locator)
                
                # 2. Verify state is clear
                initial_output = _read_output(is_chat, output_locator)
                if initial_output != "":
                ...
                # 5. Wait for output to appear (and be different from empty)
                actual_output = ""
                tries = max(2, int(args.retries))
                wait_step = max(500, int(args.retry_wait_ms))
                
                for i in range(tries):
                    page.wait_for_timeout(wait_step)
                ...
                if not actual_output:
                    actual_output = "(No output produced by the translator)"

                _set_cell_value(ws, row_index, actual_col_idx, actual_output)

                if expected_sinhala:
                    # Basic normalization for comparison
                    norm_actual = re.sub(r'\s+', ' ', actual_output).strip()
                    norm_expected = re.sub(r'\s+', ' ', expected_sinhala).strip()
                    status = "PASS" if norm_actual == norm_expected else "FAIL"
                else:
                    status = "COLLECTED"
                
                _set_cell_value(ws, row_index, status_col_idx, status)
                print(f"  -> {status} (Output: {actual_output[:30]}...)")
                processed += 1
